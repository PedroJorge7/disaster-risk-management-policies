...
dados2 <- dados %>% filter(mun_affected > 0) %>% 
  mutate(share_direct = (injured + deceased + sick + missing_people)/pop,
         desalojados = share_desalojados*pop,
         log_desalojados = log(desalojados+1))
...
eq1_control_nivel <- feols(desalojados ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | map_risco_inund ~ z_mean_map_risco_inund,
                     se="hetero", data=dados2)
results1_control_nivel <- table_ivreg(eq1_control_nivel)

eq2_control_nivel <- feols(desalojados  ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | fisc_ench ~ z_mean_fisc_ench,
                     se="hetero", data=dados2)
results2_control_nivel <- table_ivreg(eq2_control_nivel)


eq3_control_nivel <- feols(desalojados  ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | sist_alert_ench ~ z_mean_sist_alert_ench,
                     se="hetero", data=dados2)
results3_control_nivel <- table_ivreg(eq3_control_nivel)

eq4_control_nivel <- feols(desalojados  ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | prog_hab_realoc ~ z_mean_prog_hab_realoc,
                     se="hetero", data=dados2)
results4_control_nivel <- table_ivreg(eq4_control_nivel)

eq1_control_log <- feols(log_desalojados ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | map_risco_inund ~ z_mean_map_risco_inund,
                     se="hetero", data=dados2)
results1_control_log <- table_ivreg(eq1_control_log)

eq2_control_log <- feols(log_desalojados  ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | fisc_ench ~ z_mean_fisc_ench,
                     se="hetero", data=dados2)
results2_control_log <- table_ivreg(eq2_control_log)


eq3_control_log <- feols(log_desalojados  ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | sist_alert_ench ~ z_mean_sist_alert_ench,
                     se="hetero", data=dados2)
results3_control_log <- table_ivreg(eq3_control_log)

eq4_control_log <- feols(log_desalojados  ~ cobertura_agua +
                       p_informal + p_urbana + log_gdp_pc + log_population + count_100mm  | prog_hab_realoc ~ z_mean_prog_hab_realoc,
                     se="hetero", data=dados2)
results4_control_log <- table_ivreg(eq4_control_log)


results <- cbind(results1_control_nivel,results1_control_log,
                 results2_control_nivel,results2_control_log,
                 results3_control_nivel,results3_control_log) %>% data.frame
...
write.csv2(results,"./results/tables/04_robustness_check_alternative_measure.csv",row.names =  F)
